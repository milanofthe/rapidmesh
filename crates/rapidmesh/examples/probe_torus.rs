...
fn main() {
    rapidmesh_exact::log::set_level(rapidmesh_exact::log::Level::Info);
    let mut s = Scene::new();
    ...
    for (i, f) in b.faces.iter().enumerate() {
        println!("  face {i}: loops {} facets {} surface {:?}", f.loops.len(), f.facets.len(), plc.surfaces[f.plc_surface as usize]);
    }
