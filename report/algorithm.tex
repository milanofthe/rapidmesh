...
exceeds $\varepsilon$ \emph{or} a segment exceeds $h_{\max}$.
...
arc length on the surface. \Cref{tab:fff} lists $E,F,G,\dd A$ for every analytic
...
(\cref{tab:curv}) confirm the geometric intuition: developables have
$K=0$, the sphere is umbilic.

\begin{table}[H]\centering
\caption{Principal/Gaussian curvature per modality (verified by computer
algebra). $R_{\min}=1/\kappa_{\max}$ feeds the surface deflection bound.}
\label{tab:curv}
\begin{tabular}{@{}llll@{}}
\toprule
...
to the cone's first fundamental form (\cref{tab:fff}), i.e.\ \emph{exactly
isometric}.
