...
% Defined for FIGURES and PLOTS only; the document itself is classic LaTeX
% (black Computer Modern, no coloured headings). Plot/figure colour cycle:
% accent, amber, green, purple, blue, olive.
...
reproducibility (\S\ref{sec:notation}). The three barriers (edges $\to$ faces
...
\Repeat
  ...
  \State $e^\star,s^\star \gets$ worst chord--arc deflection and its location
         \Comment{error-driven}
  \State $\ell^\star,m^\star \gets$ longest segment length and its midpoint
         \Comment{$h_{\max}$-driven}
  \If{$e^\star > \varepsilon$ \textbf{or} $\ell^\star > h_{\max}$}
       insert a site (at $s^\star$ if $e^\star{>}\varepsilon$, else $m^\star$);
       $n\!\mathrel{+}\!=\!1$ \EndIf
\Until{$e^\star \le \varepsilon$ \textbf{and} $\ell^\star \le h_{\max}$
       \textbf{and} max site move $<\tau_{\text{move}}$}
...
Three chart families cover all modalities (\S\ref{sec:charts}): \emph{planar}
...
\Repeat
  ...
  \State $e^\star \gets \max$ triangle deflection vs.\ $\vv{S}$
         (sagitta, \eqref{eq:surf_defl}) \Comment{error-driven}
  \State $\ell^\star \gets \max$ triangle edge length (in arc length)
         \Comment{$h_{\max}$-driven}
  \If{$e^\star>\varepsilon$ \textbf{or} $\ell^\star>h_{\max}$}
     insert a site at the offending triangle's circumcentre (in the chart)
     \EndIf
\Until{$e^\star\le\varepsilon$ \textbf{and} $\ell^\star\le h_{\max}$
       \textbf{and} max move $<\tau_{\text{move}}$}
...
(Principle~\ref{prin:parallel}); a shared interface facet is meshed once on
$\mathcal{S}$ and both regions tetrahedralize against the same fixed triangles,
which keeps the multi-material mesh conforming across interfaces.
