...
% ============================================================================
The algorithm is validated on a from-scratch corpus of roughly ninety geometries
generated directly through the geometry API (no pre-baked tessellations), spanning
eight families: \emph{2D plates} (the planar surface stage in isolation),
...
sweeps, \emph{showcase} models, and \emph{RF/EM} structures. Each geometry is
...
\medskip\noindent
\textbf{Summary.} 89 of 90 geometries mesh (one RF assembly is a degenerate
input). All 23 analytic-primitive and box-derived (sizing, gradation) volume
meshes are watertight and free of slivers and straddlers, at the expected
isotropic quality (minimum dihedral typically $12$--$20^\circ$). On the harder
...
\subsection{Gallery}
\label{sec:gallery}
For each geometry the \textsc{normal} view (left: region-coloured solid, volumes
clipped in $y$ to expose the interior tetrahedra) sits beside the \textsc{debug}
view (right: surface wireframe, located defect markers, and a metrics +
meshing-time overlay).
