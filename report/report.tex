...
\noindent
Boundary recovery is by \emph{bistellar flips} \cite{ShewchukCDT2002,Si2015}: a
constraint edge or facet absent from the Delaunay is pierced by a tetrahedron
edge, and a local $2$-$3$ or $3$-$2$ flip removes that edge so the constraint
appears as a union of faces (the $3$D analogue of the $2$D constrained-edge flip
of \cref{sec:faces}). \emph{Planar facets need no recovery}: their vertices are
coplanar, so the Delaunay restricted to the plane already tiles them with a
tetrahedron on each side; recovery is confined to curved and feature regions, and
the flip count is therefore small. The rare configuration that no flip resolves
(a Schönhardt-type lock) is split by a Steiner point lying \emph{on}
$\mathcal{S}$, constructed on its carrier (a point on a line, $\vv{x}=\vv a+t(\vv
b-\vv a)$, or on a plane, $\vv{x}=\vv a+u(\vv b-\vv a)+v(\vv c-\vv a)$, so it
stays exactly on the carrier under repeated splitting); the boundary geometry, and
a planar region's bit-exact volume, are thereby preserved and
\cref{prop:watertight} still holds.

\begin{algorithm}[H]
\caption{\textsc{RecoverFacets}$(\mathcal{T},\mathcal{S})$: flip-based boundary
recovery}
\label{alg:recover}
\KwIn{Delaunay $\mathcal{T}$ of (interior $\cup$ surface vertices); constraint
      surface $\mathcal{S}$ (planar facets exempt, \cref{prop:watertight})}
\KwOut{CDT containing every non-planar $\triangle\in\mathcal{S}$ as a union of faces}
\ForEach{constraint edge $e\in\mathcal{S}$ not yet a union of mesh edges}{
  \lWhile{a tet edge crosses $e$}{$2$-$3$/$3$-$2$ flip it away}
  \lIf{still missing}{split $e$ at a Steiner point on its carrier}}
\ForEach{constraint facet $\triangle\in\mathcal{S}$ not yet a union of faces}{
  flip the tet edge piercing $\triangle$ so $\triangle$ becomes a union of faces;
  \hfill\tcp*[h]{Steiner on $\mathcal{S}$ only if no flip applies}}
\Return $\mathcal{T}$\;
\end{algorithm}
...
performs only a bounded, in practice tiny, number of flips (a Steiner split is
rarer still), and slivers left for the cleanup pass are rare
...
% ============================================================================
The algorithm is validated on a from-scratch corpus generated directly through
the geometry API (no pre-baked tessellations): \emph{2D plates} (the planar
surface stage in isolation), \emph{3D primitives}, and \emph{booleans} (unions
and void cuts) in many combinations, each meshed at three densities (coarse /
medium / fine). For every case we record the element quality (the minimum
planar angle for the 2D plates, the minimum dihedral angle for the volumes)
and the element count (\cref{tab:validation}); the
...
wrapper of \cref{sec:notation} (\cref{fig:gallery}). The 3D cases are
\emph{clipped at 70\% in $y$} so the interior tetrahedra are visible.

\begin{figure}[p]\centering
  \newcommand{\valfig}[2]{%
    \begin{subfigure}[t]{0.315\textwidth}\centering
      \includegraphics[width=\linewidth]{val/#1.png}\\[-3pt]
      {\footnotesize #2}\end{subfigure}}
  \valfig{disc}{disc (2D)}\hfill\valfig{hexagon}{hexagon (2D)}\hfill
  \valfig{pentagon}{pentagon (2D)}\\[6pt]
  \valfig{sphere}{sphere}\hfill\valfig{cylinder}{cylinder}\hfill
  \valfig{cone}{cone}\\[6pt]
  \valfig{torus}{torus}\hfill\valfig{hex_prism}{hexagonal prism}\hfill
  \valfig{wedge}{wedge}\\[6pt]
  \valfig{fused_two}{2 fused spheres}\hfill\valfig{capsule}{capsule}\hfill
  \valfig{tube}{hollow tube}\\[6pt]
  \valfig{via}{via (2 regions)}\hfill\valfig{drilled_block}{drilled block}\hfill
  \valfig{cross_cyl}{crossed cylinders}
  \caption{Validation gallery (medium density), rendered headlessly through the
  \emph{unchanged} viewer on a transparent background. 2D plates (row 1); 3D
  primitives (rows 2--3) and booleans (rows 4--5) are clipped at 70\% in $y$ to
  expose the interior tetrahedra. The full quality matrix is in
  \cref{tab:validation}.}
  \label{fig:gallery}
\end{figure}
...
\noindent
\textbf{Quality.} The flat plates and the smooth primitives reach the expected
isotropic quality (min dihedral $\approx 15$--$22^\circ$; min planar angle
$\approx 30$--$34^\circ$), independent of density, and unions of primitives stay
in the same band: the centroidal relaxation supplies the well-shaped point set
and the sliver-removal pass (\cref{sec:volume}) clears the residual bad elements.
Conformity does not depend on density at all: the boundary-constrained
tetrahedralization (\cref{prop:watertight}) makes the surface a hard constraint,
so each region's boundary is exactly the prescribed subset of $\mathcal{S}$ with
no straddling tetrahedra. Two ingredients carry this through the harder cases the
corpus exercises: the \emph{local-thickness} sizing (\cref{eq:thickness})
refines thin features (the cone apex, the torus tube, slender void walls) so the
interior is resolved before they can pinch; and enforcing the boundary loops as
constraints (the 2D analogue of \cref{prop:watertight} at the face stage)
recovers non-convex and holed plates (an \emph{unconstrained} triangulation would
fill their convex hull). The validation harness (\code{report/validate.py})
re-runs the whole matrix on demand, so the corpus is a standing regression gate.
